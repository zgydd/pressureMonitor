...
var Heatmapagent = (function heatmapagentClosure() {
    ...
    function Heatmapagent(productInfo) {
        ...
        this.__HEATMAPCONFIG__.baseRadius = Math.min(~~(window.screen.availHeight / (productInfo.size.x + 1)), ~~((window.screen.availWidth * 0.7) / (productInfo.size.y + 1)));
        this.__DOM__ = $('<div></div>').addClass('viewer');
        if (this.__PRODUCTCONFIG__.hasOwnProperty('deformed')) {
            this.__DOM__.addClass('deformed-' + this.__PRODUCTCONFIG__.deformed);
            $('main>div.left>article.heatmap-container>.human').addClass('deformed-' + this.__PRODUCTCONFIG__.deformed);
        }
        this.__DOM__.css({
        ...
        setTimeout(function() {
            ...
            that.createGrid();
            $('main>div.left>article.heatmap-container>.heatmap-overlay').css({
                'left': $('main>div.left>article.heatmap-container>.container').offset().left,
                'top': $('main>div.left>article.heatmap-container>.container').offset().top
            });
            //$('main>div.left>article.heatmap-container>.heatmap-overlay').innerWidth()
    ...
    Heatmapagent.prototype = {
        createGrid: function() {
            ...
            content.fillStyle = "rgba(255,255,255,0.6)";
            content.font = "16px Arial";
            content.strokeStyle = "rgba(255,117,0,0.3)";
            ...
            for (cursor; cursor < cavGrid.height - offset; cursor += this.__HEATMAPCONFIG__.baseRadius) {
                ...
                if (ctxIdx > 0 && ctxIdx % 5 === 0) {
                    var mark = '';
                    if (ctxIdx < 10) mark = ' ';
                    content.fillText(mark + ctxIdx, 0, (cursor - this.__HEATMAPCONFIG__.baseRadius / 2 + 8));
                }
                ctxIdx++;
            }
            content.fillText(ctxIdx, 0, (cursor - this.__HEATMAPCONFIG__.baseRadius / 2 + 8));
            for (cursor; cursor < cavGrid.height; cursor += this.__HEATMAPCONFIG__.baseRadius) {
                content.moveTo(0, cursor + 1);
                content.lineTo(cavGrid.width, cursor + 1);
            }
            for (cursor = offset; cursor >= 0; cursor -= this.__HEATMAPCONFIG__.baseRadius) {
                content.moveTo(0, cursor + 1);
                content.lineTo(cavGrid.width, cursor + 1);
            }
            ...
            for (cursor = offset; cursor < cavGrid.width - offset; cursor += this.__HEATMAPCONFIG__.baseRadius) {
                ...
                if (ctxIdx > 0 && ctxIdx % 5 === 0) {
                    var mark = '';
                    if (ctxIdx < 10) mark = ' ';
                    content.fillText(mark + ctxIdx, (cursor - this.__HEATMAPCONFIG__.baseRadius / 2 - 9), 16);
                }
                ctxIdx++;
            }
            content.fillText(ctxIdx, (cursor - this.__HEATMAPCONFIG__.baseRadius / 2 - 9), 16);
            content.fillText(' 0', 0, 16);
            content.stroke();
            content.save();
            this.__DOM__.append($('<img src="./assets/images/pressure.png" />'));
        },
        _getDom: function() {
            return this.__DOM__;
        },
        _repaint: function(productConfig) {
            ...
            if (!points.length) $('main>div.left>article.heatmap-container>.container>.viewer>img').removeClass('hidden');
            else $('main>div.left>article.heatmap-container>.container>.viewer>img').addClass('hidden');
