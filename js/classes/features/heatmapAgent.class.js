...
var Heatmapagent = (function heatmapagentClosure() {
    ...
    function Heatmapagent(productInfo) {
        ...
        this.__HEATMAPCONFIG__.baseRadius = Math.min(~~((window.screen.availHeight - 64) / (productInfo.size.x + 1)), ~~((window.screen.availWidth - 64) / (productInfo.size.y + 1)));
        this.__DOM__ = $('<div></div>').addClass('viewer');
        if (this.__PRODUCTCONFIG__.hasOwnProperty('deformed')) this.__DOM__.addClass('deformed-' + this.__PRODUCTCONFIG__.deformed);
        this.__DOM__.css({
    ...
    Heatmapagent.prototype = {
        createGrid: function() {
            ...
            content.fillStyle = "rgba(255,255,255,0.5)";
            ...
            /*
            for (cursor; cursor < cavGrid.width; cursor += this.__HEATMAPCONFIG__.baseRadius) {
                content.moveTo(cursor, 0);
                content.lineTo(cursor, cavGrid.height);
            }
            for (cursor = offset; cursor >= 0; cursor -= this.__HEATMAPCONFIG__.baseRadius) {
        ...
        _getImageData: function() {
            try {
                ...
                screenContext.drawImage(this.__DOM__.children('.heatmap-grid').get(0), 0, 0);
                return result;
            } catch (e) {
                console.log(e);
                return null;
